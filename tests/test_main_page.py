...
class TestMainPage:
    @allure.description('Проверка, что по клику на "Конструктор" отображается форма контсруктора')
    def test_constructor_visibility(self, driver):
        main_page = MainPage(driver)
        assert main_page.show_constructor_form() == 'Соберите бургер'

    @allure.description('Проверка, что по клику на "Лента заказов" отображается лента заказов.')
    ...
    @allure.description('Проверка, что по клику на ингридиент, появляется всплывающее окно с деталями.')
    def test_ingredient_modal_window_is_open(self, driver):
        main_page = MainPage(driver)
        assert main_page.show_modal_ingredient() == 'Детали ингредиента'

    @allure.description('Проверка, что всплывающее окно с деталями закрывается кликом по крестику.')
